-- Upgrade source: docs/sql/dependent-email-recipient-configuration.sql
-- Database-driven recipient routing for DEPENDENT_MANAGEMENT emails.
-- DEPENDENT_REJECTED remains inactive because the current system sends an SMS, not an email.
...
VALUES
    ('DEPENDENT_SUBMITTED', 'DEPENDENT_MANAGEMENT', 'Dependent submitted and pending HR approval', 'ACTIVE', 'system', 'system'),
    ('DEPENDENT_APPROVED', 'DEPENDENT_MANAGEMENT', 'Dependent approved by HR', 'ACTIVE', 'system', 'system'),
    ('DEPENDENT_REJECTED', 'DEPENDENT_MANAGEMENT', 'Dependent rejected by HR; current delivery channel is SMS', 'INACTIVE', 'system', 'system')
...
WHERE code = 'DEPENDENT_SUBMITTED';
...
WHERE event.code = 'DEPENDENT_APPROVED';
