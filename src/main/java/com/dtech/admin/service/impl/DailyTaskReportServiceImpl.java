...
public class DailyTaskReportServiceImpl implements DailyTaskReportService {
    ...
    private DailyTaskDdfRowDTO buildDdfRow(DateRange dateRange, DailyTaskReportSearchDTO search) {
        ...
        DailyTaskReportStageDTO haveToCompleteFinalCheck = stageFromDeathClaims(allClaims.stream()
                .filter(claim -> Workflow.UNDER_REVIEW.equals(claim.getRequestStatus()))
        ...
        List<DeathClaimRequest> finalDecisionClaims = allClaims.stream()
                .filter(claim -> Set.of(Workflow.APPROVED, Workflow.REJECTED).contains(claim.getRequestStatus()))
                .toList();
    ...
    private DailyTaskReportStageDTO stageDdfPaymentsCompleted(List<PaymentAdviceDeathClaim> adviceClaims) {
        ...
        long paymentAdviceTotal = safeAdviceClaims.stream()
                .map(PaymentAdviceDeathClaim::getPaymentAdvice)
                .filter(Objects::nonNull)
                .map(PaymentAdvice::getId)
                .filter(Objects::nonNull)
                .distinct()
                .count();
        long rejectedClaimTotal = safeAdviceClaims.stream()
    ...
    private int writeDdfSection(Sheet sheet,
        ...
        String[] headers = {
                ...
                "Prepare Payment", "Payments Completed", "Other Works"
        };
        writeHeaders(headerRow, headers, headerStyle);
        ...
        writeCell(row, 9, stageText(rowDTO.getPaymentsCompleted()), bodyStyle);
        writeCell(row, 10, rowDTO.getOtherWorks(), bodyStyle);
        return startRow;
    }
