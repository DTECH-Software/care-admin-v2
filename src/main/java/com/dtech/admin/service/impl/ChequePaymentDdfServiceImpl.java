...
public class ChequePaymentDdfServiceImpl implements ChequePaymentDdfService {

    private static final String PAGE_CREATE = "RPLP_CRE";
    private static final String PAGE_VIEW = "RPLP_VIW";
    private static final String DEFAULT_STAFF_CATEGORY = "ALL";

    @Autowired
    private final ChequePaymentDdfRepository chequePaymentRepository;
    ...
    public ResponseEntity<ApiResponse<Object>> getReferenceData(ChannelRequestDTO channelRequestDTO, Locale locale) {
        try {
            log.info("DDF cheque payment reference data {}", channelRequestDTO);
            ...
            log.error("Failed to load DDF cheque payment reference data", e);
            throw e;
        }
    }
    ...
    public ResponseEntity<ApiResponse<Object>> create(ChequePaymentCreateDTO createDTO, Locale locale) {
        try {
            log.info("DDF cheque payment create {}", createDTO);
            ...
            ChequePaymentDdf payment = new ChequePaymentDdf();
            payment.setCompanyCode(createDTO.getCompany());
            payment.setStaffCategoryCode(DEFAULT_STAFF_CATEGORY);
            ...
            Map<String, String> companyDescriptions = loadCompanyDescriptions();
                ChequePaymentResponseDTO responseDTO = mapToResponse(payment, companyDescriptions, true);
            ...
            log.error("Failed to create DDF cheque payment", e);
            throw e;
        }
    }
    ...
    public ResponseEntity<ApiResponse<Object>> filterList(PaginationRequest<ChequePaymentSearchDTO> paginationRequest, Locale locale) {
        try {
            ...
            Page<ChequePaymentDdf> page = chequePaymentRepository.findAll(
                    ChequePaymentDdfSpecification.getSpecification(search), pageable);
            long total = chequePaymentRepository.count(ChequePaymentDdfSpecification.getSpecification(search));

            Map<String, String> companyDescriptions = loadCompanyDescriptions();
            List<ChequePaymentResponseDTO> rows = page.stream()
                    .map(payment -> mapToResponse(payment, companyDescriptions, true))
                    .toList();
            ...
            log.error("Failed to filter DDF cheque payments", e);
            throw e;
        }
    }
    ...
    public ResponseEntity<ApiResponse<Object>> view(ChannelRequestDTO requestDTO, Long id, Locale locale) {
        try {
            log.info("DDF cheque payment view {}", id);
            ...
            log.error("Failed to view DDF cheque payment {}", id, e);
            throw e;
        }
    }
    ...
    public ResponseEntity<byte[]> export(PaginationRequest<ChequePaymentSearchDTO> paginationRequest, Locale locale) {
        try {
            ...
            List<ChequePaymentDdf> rows = Objects.nonNull(search)
                    ? chequePaymentRepository.findAll(ChequePaymentDdfSpecification.getSpecification(search))
                    : chequePaymentRepository.findAll(ChequePaymentDdfSpecification.getSpecification());

            Map<String, String> companyDescriptions = loadCompanyDescriptions();
            byte[] excelBytes = buildExcel(rows, companyDescriptions);
            ...
            String fileName = "ddf-cheque-payment-report.xlsx";
            return ResponseEntity.ok()
                    .contentType(MediaType.parseMediaType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + fileName + "\"")
                    .body(excelBytes);
        } catch (Exception e) {
            log.error("Failed to export DDF cheque payments", e);
            throw e;
        }
    }
    ...
    private Document uploadDocument(ChequePaymentDocumentDTO doc) {
        ...
        try {
            ...
            log.error("Failed to upload DDF cheque document {}", doc.getFileName(), e);
    ...
    private ChequePaymentResponseDTO mapToResponse(ChequePaymentDdf payment,
    ...
    private byte[] buildExcel(List<ChequePaymentDdf> rows,
                              Map<String, String> companyDescriptions) {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Sheet sheet = workbook.createSheet("DDF Cheque Payments");
            ...
            titleCell.setCellValue("DDF Cheque Payments");
            ...
            createStringCell(row, 3, "Months", headerStyle);
            createStringCell(row, 4, "Cheque No", headerStyle);
            createStringCell(row, 5, "Cheque Bank", headerStyle);
            createStringCell(row, 6, "Cheque Branch", headerStyle);
            createStringCell(row, 7, "Cheque Date", headerStyle);
            createStringCell(row, 8, "Amount", headerStyle);
            createStringCell(row, 9, "Received Date", headerStyle);

            int lineNo = 1;
            for (ChequePaymentDdf payment : rows) {
                row = sheet.createRow(rowIndex++);
                ...
                createStringCell(row, 2, payment.getYear(), dataStyle);
                createStringCell(row, 3, String.join(", ", payment.getMonths()), dataStyle);
                createStringCell(row, 4, payment.getChequeNo(), dataStyle);
                createStringCell(row, 5, payment.getChequeBank(), dataStyle);
                createStringCell(row, 6, payment.getChequeBranch(), dataStyle);
                createStringCell(row, 7, formatDate(payment.getChequeDate()), dataStyle);
                createStringCell(row, 8, payment.getAmount() != null ? payment.getAmount().toPlainString() : "", dataStyle);
                createStringCell(row, 9, formatDate(payment.getReceivedDate()), dataStyle);
            }

            workbook.write(out);
            return out.toByteArray();
        } catch (Exception e) {
            throw new RuntimeException("Failed to generate DDF cheque payment excel", e);
        }
    }
