...
@RestController
@RequestMapping(path = "api/v1/claims-approval")
@Log4j2
...
public class ClaimApprovalController {
    ...
    @ApiOperation(value = "Handle claim approval details reference data find request request ",notes = "Claim approval details reference data request success or failed")
    public ResponseEntity<ApiResponse<Object>> getReferenceDate(@RequestBody @Valid ChannelRequestValidatorDTO channelRequestValidatorDTO, Locale locale) {
        ...
        return approvalService.getReferenceDate(gson.fromJson(gson.toJson(channelRequestValidatorDTO), ChannelRequestDTO.class), locale);
    }

    @PostMapping(path = "/filter-list",produces = MediaType.APPLICATION_JSON_VALUE,consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Handle claim approval details filter list request request ",notes = "Claim approval details filter list request success or failed")
    public ResponseEntity<ApiResponse<Object>> filterList(@RequestBody @Valid PaginationRequest<ClaimRequestSearchDTO> paginationRequest, Locale locale) {
        ...
        return approvalService.filterList(gson.fromJson(gson.toJson(paginationRequest), paginationRequestType), locale);
    }

    @PostMapping(path = "/view",produces = MediaType.APPLICATION_JSON_VALUE,consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Handle claim approval details find by ID request request ",notes = "Claim approval details find by ID request success or failed")
    public ResponseEntity<ApiResponse<Object>> view(@RequestBody @Validated(OnGet.class) @Valid ClaimRequestRequestValidatorDTO claimRequestRequestValidatorDTO, Locale locale) {
        ...
        return approvalService.view(gson.fromJson(gson.toJson(claimRequestRequestValidatorDTO), ClaimRequestDTO.class), locale);
    }

    @PostMapping(path = "/update",produces = MediaType.APPLICATION_JSON_VALUE,consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Handle claim approval details update request request ",notes = "Claim approval details update request success or failed")
    public ResponseEntity<ApiResponse<Object>> actionRequest(@RequestBody @Validated(OnUpdate.class) @Valid ClaimRequestRequestValidatorDTO claimRequestRequestValidatorDTO, Locale locale) {
        log.info("Claim approval update request  controller {} ", claimRequestRequestValidatorDTO);
        return approvalService.actionRequest(gson.fromJson(gson.toJson(claimRequestRequestValidatorDTO), ClaimRequestDTO.class), locale);
    }
