...
@RestController
@RequestMapping(path = "api/v1/reports/claim-status")
@Log4j2
...
public class ClaimStatusReportController {
    ...
    @Autowired
    private final ClaimStatusReportService reportService;

    @Autowired
    private final Gson gson;

    @PostMapping(path = "/reference-data", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Handle claim status report reference data request", notes = "Claim status report reference data request success or failed")
    public ResponseEntity<ApiResponse<Object>> getReferenceDate(@RequestBody @Valid ChannelRequestValidatorDTO validatorDTO,
                                                                Locale locale) {
        log.info("Claim status report reference data request {}", validatorDTO);
    ...
    @ApiOperation(value = "Handle claim status report filter list request", notes = "Claim status report filter list request success or failed")
    ...
    @ApiOperation(value = "Handle claim status report export request", notes = "Claim status report export request success or failed")
